...
from spotjax.utils.logging import print_error, print_status, print_success, print_warning
...
async def run_setup(fix: bool = False) -> bool:
    ...
    from spotjax.utils.logging import console
...
def check_prerequisites_quick() -> tuple[bool, list[str]]:
    """Quick prerequisite check for use before 'spotjax run'.
    ...
    if not ssh_key_found:
        errors.append("SSH key not found. Run: spotjax setup --fix")
